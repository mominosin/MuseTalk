...
> **重要**: 動画ファイルを入力した場合、一部のパラメータは無視されます。

| パラメータ | 動画入力 | 画像入力 | 説明 |
|-----------|:-------:|:-------:|------|
| `--fps` | ❌ **無視** | ✅ 有効 | 動画入力時は入力動画のFPSが使用される |
| `--bbox_shift` (v1.5) | ❌ 固定(0) | ❌ 固定(0) | v1.5では常に0固定 |
| `--bbox_shift` (v1.0) | ✅ 有効 | ✅ 有効 | v1.0でのみ調整可能 |
| その他パラメータ | ✅ 有効 | ✅ 有効 | - |

#### 動画入力時に品質に影響するパラメータ（v1.5）

| パラメータ | デフォルト | 範囲 | 効果 |
|-----------|-----------|------|------|
| `--extra_margin` | 10 | 0～50 | 顔切り抜き領域の下方向拡張。**顎切れ対策に重要** |
| `--parsing_mode` | jaw | jaw/face | 合成領域。**境界の自然さに影響** |
| `--left_cheek_width` | 90 | 50～150 | 左頬のブレンド幅。**境界の目立ちやすさに影響** |
| `--right_cheek_width` | 90 | 50～150 | 右頬のブレンド幅。**境界の目立ちやすさに影響** |
| `--audio_padding_length_left` | 2 | 0～5 | 音声先読み。**リップシンクのタイミングに影響** |
| `--audio_padding_length_right` | 2 | 0～5 | 音声後読み。**リップシンクのタイミングに影響** |

#### 速度・メモリに影響するパラメータ

| パラメータ | デフォルト | 範囲 | 効果 |
|-----------|-----------|------|------|
| `--use_float16` | False | フラグ | **VRAMを約50%削減**、速度10-20%向上 |
| `--batch_size` | 8 | 1～32+ | **大きいほど高速だがVRAM消費増** |
| `--gpu_id` | 0 | 0～N | 使用するGPU番号 |

#### yaml設定で指定可能なパラメータ

`configs/inference/test.yaml` でタスクごとに指定できるパラメータ：

| パラメータ | v1.0 | v1.5 | 説明 |
|-----------|:----:|:----:|------|
| `video_path` | ✅ | ✅ | 入力動画/画像パス |
| `audio_path` | ✅ | ✅ | 入力音声パス |
| `result_name` | ✅ | ✅ | 出力ファイル名（上書き防止に有効） |
| `bbox_shift` | ✅ | ❌ | 口の開き具合（v1.0のみ） |

> **注意**: `extra_margin`, `parsing_mode`, `cheek_width`等はyamlで指定できません。コマンドライン引数でのみ指定可能です。
...
> **重要**: `--fps`パラメータは**画像入力時のみ有効**です。

| 入力タイプ | 出力FPS |
|-----------|---------|
| **動画ファイル** | 入力動画のFPSがそのまま使用される |
| 画像ファイル | `--fps`で指定した値 |
| 画像ディレクトリ | `--fps`で指定した値 |

動画のFPSを変更したい場合は、事前にffmpegで変換してください：

```bash
# 入力動画を25fpsに変換
ffmpeg -i input.mp4 -r 25 -c:v libx264 -crf 18 input_25fps.mp4
```
...
| 問題 | 調整するパラメータ | 推奨値 |
|------|------------------|--------|
| **顎が切れる** | `--extra_margin` | 15～30に増やす |
| **口周辺が不自然** | `--parsing_mode` | `face`に変更 |
| **頬の境界が目立つ** | `--left/right_cheek_width` | 80～110で調整 |
| **リップシンクがずれる** | `--audio_padding_length_*` | 0～5で調整 |
| **OOMエラー** | `--batch_size`, `--use_float16` | 小さく、有効化 |
